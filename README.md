...
- **Inline allowlist:** a line carrying `# pragma: allowlist secret` (the marker Yelp's detect-secrets uses) is skipped entirely and never written into a baseline. Use it for fixture values in tests; anything in real configuration belongs in the environment, not behind a pragma.
...
### `gitignore-shadow-check`

A gitignore pattern with no slash matches a file *or directory* of that name at any depth below the `.gitignore`, not just the file it was written for. If a tracked directory happens to share that name, every new file created under it is silently ignored: it never shows up in `git status`, `git add .` skips it with no error, and already-tracked files inside it keep working normally, so nothing looks wrong. This hook catches that before it costs anyone a debugging session.

For each `.gitignore` given, it checks every pattern that has no anchoring slash (comments, blank lines and negations are skipped) against the tracked directories below that `.gitignore`, and fails if one matches.

**What counts as unanchored:** a pattern with no `/`, or with a `/` only at the very end (`core/` is exactly as unanchored as `core` — [gitignore(5)](https://git-scm.com/docs/gitignore) only anchors a pattern when the slash appears at the start or in the middle). Anything with a `/` elsewhere is already scoped to a specific path and is not checked.

**Wildcards are matched, not skipped.** A bare `build*` is checked against real tracked directory names the same way git would match it, so it is flagged if a `build` directory exists and left alone otherwise. Skipping every wildcard pattern outright would let a genuinely dangerous one like that through, while a harmless pattern such as `*.core` never matches a real directory name and stays silent regardless.
...
```
.gitignore:214: 'core' matches tracked directory: datasphere/core
```
