...
def test_foreign_baseline_is_rejected_not_silently_trusted(tmp_path, run_hook):
    """A baseline generated by the separate, real detect-secrets PyPI package
    has a totally different shape (version/plugins_used/results with
    SHA1-based entries from different detection plugins) to what this hook
    produces. Matching against it would practically never suppress anything,
    while looking configured and safe. The hook must recognise that shape
    and say so loudly rather than pretend the baseline works."""
    foreign_baseline = _write(
        tmp_path,
        ".secrets.baseline",
        json.dumps(
            {
    ...
    result = run_hook(
        "detect_secrets",
        ["config.py", "--baseline", str(foreign_baseline)],
        cwd=tmp_path,
    )

    assert result.returncode == 1
    assert "does not look like it was generated by this hook" in result.stdout
    assert "Generic secret assignment" in result.stdout
